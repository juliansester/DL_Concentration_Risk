...
def create_sample_actuarial(ELGD_bounds,EAD_bounds,PD_bounds,omega_bounds,obligor_bounds,B,
                            PDs = 0,PD_distribution =0,PDs_Sample = False):
    if PDs_Sample:
        #a = np.random.uniform(0,2)
        #b = np.random.uniform(0,2)
        #PD = np.minimum(np.random.beta(a, b, size=(B,obligor_bounds[1])),0.5147)
        PD = np.random.choice(PDs,size = (B,obligor_bounds[1]),p = PD_distribution)
        ...
        #NN = np.random.choice(100)
        #indices = np.random.choice(range(100),NN,replace = False)
        #omega[:,indices] = omega_bounds[1]
    else:
        ELGD = np.random.uniform(ELGD_bounds[0],ELGD_bounds[1],(B,obligor_bounds[1]))
        EAD = np.random.uniform(EAD_bounds[0],EAD_bounds[1],(B,obligor_bounds[1]))
