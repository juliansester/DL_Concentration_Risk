...
def create_sample_MtM(ELGD_bounds,EAD_bounds,rho_bounds,c_bounds,g_bounds,D_bounds,obligor_bounds,B,g_vals,g_probs):
    

    ELGD = np.random.uniform(ELGD_bounds[0],ELGD_bounds[1],(B,obligor_bounds[1]))
    theta = np.random.uniform(4,32)
    #theta = np.random.choice([10,30,100])
    EAD = np.random.exponential(scale=1/theta, size=(B,obligor_bounds[1]))
    
    
    rho = np.random.uniform(rho_bounds[0],rho_bounds[1],(B,obligor_bounds[1]))
    
    g = np.random.choice(g_vals,size = (B,obligor_bounds[1]),p = g_probs)
    ...
    D  = np.repeat(np.random.uniform(D_bounds[0],D_bounds[1],(B,1)),obligor_bounds[1],axis =1)
    c = np.repeat(np.random.uniform(c_bounds[0],c_bounds[1],(B,1)),obligor_bounds[1],axis =1)
    for i in range(B):
        N[i,:] = np.pad(np.repeat(1,N_obligors[i]),(0,obligor_bounds[1]-N_obligors[i])) # Number of Obligors    
    
    return ELGD*N, EAD*N / np.sum(EAD*N),rho*N,c*N,g*N,D*N
