...
def GA_GL(PD, # Vector of default probabillities
    ...
    if IRB:
        ...
        def to_solve(R_corr):
            ...
            return np.abs(ELGD*PD*rho*(alpha_X-1)-K)
        if minimize_rho_distance:
            R_corr = minimize(lambda x: np.linalg.norm(to_solve(x)), 
                              x0=[0.18]*len(A),
                              bounds=[[0.01,0.99]]*len(A)).x
        else:
